...
pub type FunctionXU<const N: usize, const R: usize> = fn(&Mat<f64, N, 1>, &Mat<f64, R, 1>) -> f64;

pub type StateDifferentialEquations<const N: usize, const R: usize> = Mat<FunctionXU<N, R>, N, 1>;

/// N - Number of states
/// R - Number of inputs
/// P - Number of sensors
#[derive(Debug, Clone)]
pub struct ContinuousNLTI<const N: usize, const R: usize, const P: usize> {
...
impl<const N: usize, const R: usize, const P: usize> StepNLTI<N, R> for ContinuousNLTI<N, R, P> {
    fn step(
        &mut self,
        dt: f64,
        u: Mat<f64, R, 1>,
        ...
        match self.integrator {
            Integrator::Euler => {
                for i in 0..N {
                    self.x[i] = (self.x[i] + dt * (self.dx_dt[i](&self.x, &u)))
                        .min(max_clamp[i])
                        .max(min_clamp[i]);
                }
            }
            ...
            Integrator::RK4 => {
                for i in 0..N {
                    let k1 = self.dx_dt[i](&self.x, &u);
                    let k2 = self.dx_dt[i](&self.x.add_scalar(k1 * dt / 2.0), &u);
                    let k3 = self.dx_dt[i](&self.x.add_scalar(k2 * dt / 2.0), &u);
                    let k4 = self.dx_dt[i](&self.x.add_scalar(k3 * dt), &u);
...
mod tests {
    ...
    type Mat21<T> = Mat<T, 2, 1>;
    type Mat12<T> = Mat<T, 1, 2>;

    const DX_DT: Mat21<FunctionXU<2, 2>> = Mat21::new(
        |x, _| {
            // dx_dt = -5xsin(t), x(0) = 1
            // x(t) = e^(5cost-5)
            -5.0 * x[0] * x[1].sin()
    ...
    const W: Mat21<SensorSpec> = Mat21::new(SensorSpec::new(0.0), SensorSpec::new(0.0));
    const V: Mat1<SensorSpec> = Mat1::new(SensorSpec::new(0.0));
    const C: Mat12<f64> = Mat12::new(1.0, 0.0);
    const X0: Mat21<f64> = Mat21::new(1.0, 0.0);
